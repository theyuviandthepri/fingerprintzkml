...
# CONFIGURATION
# -----------------------------
DATASET_PATH = "/home/ml/Fingerprint_zkml/dataset/SOCOFing" # Change this path
...
def main():
    ...
    # pin_memory=True speeds up transfer to GPU and reduces bottlenecking
    train_loader = DataLoader(SiameseDataset(train_imgs), batch_size=BATCH_SIZE, shuffle=True, pin_memory=True)
    val_loader = DataLoader(SiameseDataset(val_imgs, fixed_pairs=True), batch_size=BATCH_SIZE, pin_memory=True)
    ...
    for epoch in range(EPOCHS):
        ...
        for i1, i2, l in train_loader:
            i1, i2, l = i1.to(device, non_blocking=True), i2.to(device, non_blocking=True), l.to(device, non_blocking=True)
            
            optimizer.zero_grad(set_to_none=True) # Optimized memory clearing instead of zero_grad()
            e1, e2 = model(i1, i2)
            loss = criterion(e1, e2, l)
            loss.backward()
            optimizer.step()
            
            running += loss.item() # Stripping out the scaler value explicitly
        ...
        # Explicit inference mode for validation reduces memory overhead
        with torch.inference_mode(): 
            for i1, i2, l in val_loader:
                i1, i2 = i1.to(device, non_blocking=True), i2.to(device, non_blocking=True)
                e1, e2 = model(i1, i2)
                d = F.pairwise_distance(e1, e2)
                
                # Detached array handling to prevent tracking leaks
        ...
        # Actively empty cache if working on a tight VRAM budget
        if torch.cuda.is_available():
